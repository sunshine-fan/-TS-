...
class BSTree<T>{
  ...
  remove(value:T):boolean{
    ...
    //设置公共变量
    let replacrNode:TreeNode<T>|null = null
    //2.如果删除的叶子节点
    if(current.left===null&&current.right===null){
      replacrNode = null 
    }else if(current.right===null){
      replacrNode = current.left 
    }else if(current.left===null){
       replacrNode = current.right 
    }else{
      const successor = this.getSuccessor(current)
      replacrNode = successor
    }
    if(current===this.root) {
      this.root = replacrNode
    } else if(current.isLeft) {
      current.parent!.left = replacrNode
    }else{
      current.parent!.right = replacrNode
    }
    return true
  }
}
